...
export class AIWorkflowExecutor {
    ...
    while (attempt <= maxRetries) {
      try {
        ...
        const llm = await getLLM({
          model: step.model || process.env.OPEN_ROUTER_MODEL || "nvidia/nemotron-3-super-120b-a12b:free",
  ...
  async simpleAICall(
    ...
    const llm = await getLLM({
      model: options.model || process.env.OPEN_ROUTER_MODEL || "nvidia/nemotron-3-super-120b-a12b:free",
