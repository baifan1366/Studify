...
const AttemptCard = ({
  ...
  const quizPath = isTutor
    ? `/tutor/community/quizzes/${attempt.quiz.slug}`
    : `/community/quizzes/${attempt.quiz.slug}`;
  const scorePercentage =
    attempt.total_questions && attempt.total_questions > 0
      ? Math.round(
          ((attempt.correct_answers || 0) / attempt.total_questions) * 100
        )
      : 0;
          ...
          <>
            ...
            <div className="flex items-center gap-2 text-gray-300">
              <div className="flex items-center gap-1">
                ...
                <span
                  className={
                    scorePercentage >= 80 ? "text-green-400" : "text-red-400"
                  }
                >
                  {scorePercentage}%
                </span>
...
export default function AllAttemptsModal({
  isOpen,
  onClose,
}: AllAttemptsModalProps) {
  const { data: attempts, isLoading } = useAllUserQuizAttempts();
  const { data: currentUser } = useUser();
  const isTutor = currentUser?.profile?.role === "tutor";
  const quizzesPath = isTutor
  ...
  const groupedAttempts = React.useMemo(() => {
    ...
    return attempts.reduce((acc, attempt) => {
      const status =
        attempt.status === "not_started" ? "in_progress" : attempt.status;
