...
export default function GroupDetailContent({
  ...
  const { members, joinGroup, isJoining, leaveGroup, isLeaving } =
    useGroupMembers(groupSlug);
  const { hashtags, isLoading, isError, error, searchHashtags } = useHashtags();
  ...
  return (
    <>
        <div className="max-w-4xl mx-auto">
          ...
          {groupError &&
            !groupErrorMessage?.message.includes("Access denied") && (
